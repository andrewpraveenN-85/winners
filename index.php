...
<head>
  ...
  <style>
    ...
    .scroll-container {
      height: 300px; /* Exactly 3 slots visible */
      overflow: hidden;
      position: relative;
    ...
    .center-indicator {
      ...
      margin-top: -50px; /* Half of slot height */
...
<body class="bg-black min-h-screen flex items-center justify-center parallax">
  ...
  <script>
    ...
    // (scroll-container height (300px) - slot height (100px)) / 2 = 100px
    ...
    function updateNameScroller() {
      ...
      // Set initial position so that (with our CENTER_OFFSET) the correct slot will end up centered.
      // Here we start with position = -SLOT_HEIGHT so that when adjusted by CENTER_OFFSET in our math the middle slot is chosen.
    ...
    document.getElementById('startButton').addEventListener('click', () => {
      ...
      function animate() {
        ...
        if (phase === 'accelerating') {
          ...
          if (currentSpeed <= 0.2) {
            // Snap exactly to the closest slot (using the CENTER_OFFSET so that the winner is centered)
            alignToClosestSlot();
            finalizeWinner();
            return;
          }
    ...
    function alignToClosestSlot() {
      const scroller = document.getElementById('nameScroller');
      // Calculate target index by adjusting for the CENTER_OFFSET
    ...
    function finalizeWinner() {
      ...
      // Adjust position by CENTER_OFFSET so that we calculate the centered slot.
